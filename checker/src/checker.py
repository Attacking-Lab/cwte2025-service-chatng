...
async def putflag_store1(task: PutflagCheckerTaskMessage, logger: LoggerAdapter, client: AsyncClient, db: ChainDB) -> str:
    gen = RandomGenerator(seed=f"{CHECKER_ENTROPY_SECRET_SEED}|flag_store1|" + str(getattr(task, CHECKER_SEED_MOD_KEY))) if not CHECKER_USE_DB else RandomGenerator()
...
async def getflag_store1(task: GetflagCheckerTaskMessage, logger: LoggerAdapter, client: AsyncClient, db: ChainDB) -> None:
    ...
    if CHECKER_USE_DB:
        ...
        gen = RandomGenerator(seed=f"{CHECKER_ENTROPY_SECRET_SEED}|flag_store1|" + str(getattr(task, CHECKER_SEED_MOD_KEY)))
        username = gen.genStr(gen.genInt(8,12))
        password = gen.genStr(gen.genInt(12,16))
        token = await do_login(logger, client, username, password)
...
async def putflag_store2(task: PutflagCheckerTaskMessage, logger: LoggerAdapter, client: AsyncClient, db: ChainDB) -> str:
    gen = RandomGenerator(seed=f"{CHECKER_ENTROPY_SECRET_SEED}|flag_store2|" + str(getattr(task, CHECKER_SEED_MOD_KEY))) if not CHECKER_USE_DB else RandomGenerator()
    username = gen.genStr(gen.genInt(8,12))
    password = gen.genStr(gen.genInt(12,16))
...
async def putnoise_store1(task: PutnoiseCheckerTaskMessage, logger: LoggerAdapter, client: AsyncClient, db: ChainDB) -> None:
    gen = RandomGenerator(seed=f"{CHECKER_ENTROPY_SECRET_SEED}|noise_store1|" + str(getattr(task, CHECKER_SEED_MOD_KEY))) if not CHECKER_USE_DB else RandomGenerator()
    username = gen.genStr(gen.genInt(8,12))
    password = gen.genStr(gen.genInt(12,16))
    message = ''.join([
...
async def getnoise_store1(task: GetnoiseCheckerTaskMessage, logger: LoggerAdapter, client: AsyncClient, db: ChainDB) -> None:
    ...
    if CHECKER_USE_DB:
        ...
        gen = RandomGenerator(seed=f"{CHECKER_ENTROPY_SECRET_SEED}|noise_store1|" + str(getattr(task, CHECKER_SEED_MOD_KEY)))
        username = gen.genStr(gen.genInt(8,12))
        password = gen.genStr(gen.genInt(12,16))
        message = ''.join([
            gen.choice(NOISE_WELCOME_MESSAGE),
...
async def putnoise_store2(task: PutnoiseCheckerTaskMessage, logger: LoggerAdapter, client: AsyncClient, db: ChainDB) -> None:
    gen = RandomGenerator(seed=f"{CHECKER_ENTROPY_SECRET_SEED}|noise_store2|" + str(getattr(task, CHECKER_SEED_MOD_KEY))) if not CHECKER_USE_DB else RandomGenerator()
    username = gen.genStr(gen.genInt(8,12))
    password = gen.genStr(gen.genInt(12,16))
    botname = gen.choice(["grof", "genimi", "ripbard", "chatgtp", "oh4", "cocapten"]) + gen.genStr(gen.genInt(8,12))
    bottoken = gen.genStr(gen.genInt(32,64))
    ...
    if CHECKER_USE_DB:
        data = {
            ...
            "botname": botname,
            "bottoken": bottoken,
            "botnoisekey": botnoisekey,
            "botnoisevalue": botnoisevalue,
        }
...
async def getnoise_store2(task: GetnoiseCheckerTaskMessage, logger: LoggerAdapter, client: AsyncClient, db: ChainDB) -> None:
    ...
    if CHECKER_USE_DB:
        ...
        botname = data['botname']
        bottoken = data['bottoken']
        botnoisekey = data['botnoisekey']
        botnoisevalue = data['botnoisevalue']
    else:
        gen = RandomGenerator(seed=f"{CHECKER_ENTROPY_SECRET_SEED}|noise_store2|" + str(getattr(task, CHECKER_SEED_MOD_KEY)))
        username = gen.genStr(gen.genInt(8,12))
        password = gen.genStr(gen.genInt(12,16))
        botname = gen.choice(["grof", "genimi", "ripbard", "chatgtp", "oh4", "cocapten"]) + gen.genStr(gen.genInt(8,12))
        bottoken = gen.genStr(gen.genInt(32,64))
...
async def havoc_store1(task: HavocCheckerTaskMessage, logger: LoggerAdapter, client: AsyncClient) -> None:
    #gen = RandomGenerator(seed=f"{CHECKER_ENTROPY_SECRET_SEED}|havoc_store1|" + str(getattr(task, CHECKER_SEED_MOD_KEY))) if not CHECKER_USE_DB else RandomGenerator()
    gen = RandomGenerator()
...
async def havoc_store2(task: HavocCheckerTaskMessage, logger: LoggerAdapter, client: AsyncClient) -> None:
    #gen = RandomGenerator(seed=f"{CHECKER_ENTROPY_SECRET_SEED}|havoc_store2|" + str(getattr(task, CHECKER_SEED_MOD_KEY))) if not CHECKER_USE_DB else RandomGenerator()
    gen = RandomGenerator()
    username = gen.genStr(gen.genInt(8,12))
    password = gen.genStr(gen.genInt(12,16))
    botname = gen.choice(["grof", "genimi", "ripbard", "chatgtp", "oh4", "cocapten"]) + gen.genStr(gen.genInt(8,12))
...
async def exploit_store1_a(task: ExploitCheckerTaskMessage, logger: LoggerAdapter, searcher: FlagSearcher, client: AsyncClient) -> Optional[str]:
    ...
    if storetype != 'username':
        raise MumbleException("Exploit 0: Wrong flagstore")
    ...
    try:
        ...
        raise MumbleException("Exploit 0: Failed to recover inbox")
    
    flag = searcher.search_flag(msgs)
    if flag is not None:
        return flag
    raise MumbleException("Exploit 0 (session forge) for flagstore 1 failed")
...
async def exploit_store1_b(task: ExploitCheckerTaskMessage, logger: LoggerAdapter, searcher: FlagSearcher, client: AsyncClient) -> Optional[str]:
    ...
    storetype, username = task.attack_info.split(':', 1)
    if storetype != 'username':
        raise MumbleException("Exploit 1: Wrong flagstore")
    ...
    try:
        ...
        raise MumbleException("Exploit 1: Failed to recover inbox")
    
    flag = searcher.search_flag(msgs)
    if flag is not None:
        return flag
    raise MumbleException("Exploit 1 (shared code forge) for flagstore 1 failed")
...
async def exploit_store1_c(task: ExploitCheckerTaskMessage, logger: LoggerAdapter, searcher: FlagSearcher, client: AsyncClient) -> Optional[str]:
    ...
    if storetype != 'username':
        raise MumbleException("Exploit 2: Wrong flagstore")

    #gen = RandomGenerator(seed=f"{CHECKER_ENTROPY_SECRET_SEED}|exploit_store1_c|" + str(getattr(task, CHECKER_SEED_MOD_KEY)))
    gen = RandomGenerator()
    username = gen.genStr(gen.genInt(8,12))
    password = gen.genStr(gen.genInt(12,16))
    ...
    try:
        ...
        logger.error(f"Invalid search response during havoc_store1: " + f"\n{msgs}")
        raise MumbleException("Exploit 2: Invalid search response")
    ...
    if flag is not None:
        return flag
    raise MumbleException("Exploit 2 (search vuln) for flagstore 1 failed")
...
async def exploit_store2_a(task: ExploitCheckerTaskMessage, logger: LoggerAdapter, searcher: FlagSearcher, client: AsyncClient) -> Optional[str]:
    ...
    if storetype != 'botname':
        raise MumbleException("Exploit 3: Wrong flagstore")
    ...
    try:
        ...
        logger.error(f"The bot code returned is not valid:\n{e}")
        raise MumbleException("Exploit 3: Faulty bot code returned")
    ...
    if flag is not None:
        return flag
    raise MumbleException("Exploit 3 (nginx path traversal) for flagstore 2 failed")
...
async def exploit_store2_b(task: ExploitCheckerTaskMessage, logger: LoggerAdapter, searcher: FlagSearcher, client: AsyncClient) -> Optional[str]:
    ...
    try:
        ...
        logger.error(f"The bot code returned is not valid:\n{e}")
        raise MumbleException("Exploit 4: Faulty bot code returned")
    
    flag = searcher.search_flag(text)
    if flag is not None:
        return flag
    raise MumbleException("Exploit 4 (mng backdoor) for flagstore 2 failed")
...
async def exploit_store2_c(task: ExploitCheckerTaskMessage, logger: LoggerAdapter, searcher: FlagSearcher, client: AsyncClient) -> Optional[str]:
    ...
    if hacked:
        ...
        try:
            ...
            logger.error(f"The bot code returned is not valid:\n{e}")
            raise MumbleException("Exploit 5: Faulty bot code returned")
        
        flag = searcher.search_flag(text)
        if flag is not None:
            return flag
    
    raise MumbleException("Exploit 5 (mng backdoor) for flagstore 2 failed")
...
async def exploit_store2_d(task: ExploitCheckerTaskMessage, logger: LoggerAdapter, searcher: FlagSearcher, client: AsyncClient) -> Optional[str]:
    ...
    if storetype != 'botname':
        raise MumbleException("Exploit 6: Wrong flagstore")

    #gen = RandomGenerator(seed=f"{CHECKER_ENTROPY_SECRET_SEED}|exploit_store2_d|" + str(getattr(task, CHECKER_SEED_MOD_KEY)))
    gen = RandomGenerator()
    username = gen.genStr(gen.genInt(8,12))
    password = gen.genStr(gen.genInt(12,16))
    botname = gen.genStr(gen.genInt(8,12))
    ...
    try:
        ...
        raise MumbleException("Exploit 6: Failed to recover inbox")
    ...
    for code in msgs:
        ...
        try:
            ...
            logger.error(f"The bot code returned is not valid:\n{e}")
            raise MumbleException("Exploit 6: Faulty bot code returned")
    
    logger.debug(f"text: {text}")
    
    #match = re.search(r'ENO[A-Za-z0-9+\\/=]{48}', text)
    ...
    raise MumbleException("Exploit 6 (json injection) for flagstore 2 failed")
